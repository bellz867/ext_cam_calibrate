...
public:
    ...
    void imageCB(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        char key = cv::waitKey(30); // allow time to display image and get key presses
        ...
        else if (currentState_ == STATE_SAMPLING)
        {
            if (patternFound)
            {
                ...
                tfl_.waitForTransform("/world","/board",ros::Time(0),ros::Duration(0.1));
                tfl_.lookupTransform("/world","/board",ros::Time(0),boardTransform);
                ...
                tfl_.waitForTransform("/world",cameraTF_,ros::Time(0),ros::Duration(0.1));
                tfl_.lookupTransform("/world",cameraTF_,ros::Time(0),camTransform);
